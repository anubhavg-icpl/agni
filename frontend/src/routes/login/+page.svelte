	...
	<div class="card-fire max-w-md w-full p-8 relative z-10">
		...
		{#if $auth.setupRequired}
			...
			<div class="mb-6 p-4 bg-orange-500/10 border border-orange-500/30 rounded-lg">
				<h2 class="text-orange-400 font-semibold mb-1">🎉 Fresh install detected!</h2>
				<p class="text-gray-400 text-sm">Someone has to be in charge. Might as well be you.</p>
			</div>
			...
			<form on:submit|preventDefault={handleSetup} class="space-y-5">
				...
				<div>
					...
					/>
					{#if passwordTooShort}
						<p class="text-red-400 text-xs mt-1">8 characters minimum. Your cat's name doesn't count.</p>
					{/if}
				</div>
				<div>
					...
					/>
					{#if passwordMismatch}
						<p class="text-red-400 text-xs mt-1">Passwords don't match. Shocking, I know.</p>
					{/if}
				</div>
				{#if $auth.error}
		...
		{:else}
			...
			<form on:submit|preventDefault={handleLogin} class="space-y-5">
				...
				>
					{$auth.loading ? 'Verifying your existence...' : 'Grant Me Access'}
				</button>
			</form>
		{/if}

		<p class="text-center text-gray-500 text-xs mt-6">
			Powered by Firecracker, tears, and mass quantities of caffeine
		</p>
	</div>
</div>
