	...
	<div class="min-h-screen bg-gray-950 flex items-center justify-center p-4">
		<div class="bg-gradient-to-br from-gray-800/50 to-gray-900/50 border border-gray-700/50 rounded-2xl max-w-md w-full p-8">
			<div class="text-center mb-8">
				...
				<h1 class="text-2xl font-bold text-white mb-2">Fresh Meat Detected</h1>
			...
			<form on:submit|preventDefault={handleSetup} class="space-y-5">
				...
				>
					{$auth.loading ? 'Summoning powers...' : 'Crown Me Admin'}
				</button>
			</form>
		</div>
	</div>
{:else}
	<div class="space-y-6 lg:space-y-8">
		...
		<div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
			<div>
				<h1 class="text-2xl sm:text-3xl font-bold text-white mb-1">Command Center</h1>
				<p class="text-gray-400 text-sm sm:text-base">Your minions await orders. Try not to break anything.</p>
			</div>
			<a
				href="/vms/new"
		...
		<div>
			<div class="flex items-center justify-between mb-4">
				<h2 class="text-lg sm:text-xl font-semibold text-white">Your Minions</h2>
				{#if $vms.vms.length > 0}
					<span class="text-sm text-gray-500">{$vms.vms.length} {$vms.vms.length === 1 ? 'soul' : 'souls'} captured</span>
				{/if}
			</div>
			...
			{#if $vms.loading && $vms.vms.length === 0}
				<div class="bg-gradient-to-br from-gray-800/50 to-gray-900/50 border border-gray-700/50 rounded-2xl p-12">
					<div class="flex flex-col items-center justify-center text-center">
						...
						<p class="text-gray-400">Waking up the hamsters...</p>
					</div>
				</div>
			{:else if $vms.vms.length === 0}
				<div class="bg-gradient-to-br from-gray-800/50 to-gray-900/50 border border-gray-700/50 rounded-2xl p-12">
					<div class="flex flex-col items-center justify-center text-center max-w-md mx-auto">
						...
						<h3 class="text-xl font-semibold text-white mb-2">It's Empty In Here</h3>
						<p class="text-gray-400 mb-6">No VMs? How boring. Go create one and give your CPU something to cry about.</p>
						<a
							href="/vms/new"
						...
						>
							...
							<span>Summon Your First Minion</span>
						</a>
					</div>
				</div>
			{:else}
		...
		{#if $vms.error}
			<div class="bg-gradient-to-r from-red-500/10 to-red-600/10 border border-red-500/30 rounded-2xl p-4 flex items-center gap-3">
				...
				<div>
					<p class="text-red-400 font-medium">Well, That Didn't Work</p>
